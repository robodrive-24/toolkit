...
def multi_gpu_robodrive_test(model, 
                             data_loader, 
                             tmpdir=None, 
                             gpu_collect=False, 
                             show=False,
                             corruption=None,
                             out_dir='./test'):
    """Test model with multiple gpus and save results of map segmentation.
    ...
    assert not det_enable, 'det_enable should be False, only support eval map segmentation'
    assert map_enable, 'map_enable should be True'
    assert not motion_enable, 'motion_enable should be False, only support eval map segmentation'
    ...
    for i, data in enumerate(data_loader):
        ...
        assert sample_idx not in all_predict_results.keys(), 'sample_idx should not be duplicated'
        all_predict_results[sample_idx] = pred_semantic_indices
    ...
    if rank == 0:
        print('===\nNot support evaluate locally, please upload results file to server.')
    
    # Convert dictionaries to lists of tuples
    all_predict_results_list = list(all_predict_results.items())

    # Collect results across GPUs
    collected_predict_results_list = collect_results_gpu(all_predict_results_list, len(dataset))

    if rank == 0:
        # Reassemble lists of tuples back into dictionaries
        all_predict_results_collected = dict()

        for sublist in collected_predict_results_list:
            all_predict_results_collected[sublist[0]] = sublist[1].cpu().numpy()

        assert len(all_predict_results_collected) == len(dataset), 'length of all_predict_results_collected should be equal to length of dataset'
    
    if rank == 0:
        if out_dir is not None:
            if corruption is not None:
                out_dir = osp.join(out_dir, corruption)
                print(f'===\nSave Map Segmentation Results to {out_dir}')
                mmcv.mkdir_or_exist(out_dir)
                mmcv.dump(all_predict_results_collected, osp.join(out_dir, 'results.pkl'))
